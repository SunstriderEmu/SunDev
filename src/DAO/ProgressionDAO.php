...
class ProgressionDAO extends DAO
{
	...
	public function countClassesTested($category, $status) {
		$countStatus = $this->tools->fetchAssoc("SELECT (SUM(CASE WHEN tested = ? THEN 1 ELSE 0 END)) AS count FROM class_test_{$category}", array($status));
		return $countStatus['count'];
	}

	public function countClassesSpells() {
		return $this->tools->executeQuery('SELECT tested FROM class_test_spells')->rowCount();
	}

	public function countClassesTalents() {
		return $this->tools->executeQuery('SELECT tested FROM class_test_talents')->rowCount();
	}
