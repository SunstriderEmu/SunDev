...
$WH.g_bonusesBtnBuildMenu = function(e) {
    ...
    var o = e.bonusesData;
    if (o && $WH.isset("g_itembonuses")) {
        var k = [];
        for (var j in o) {
            var m = o[j].groupedUpgrade;
            var n = $WH.g_getItemBonusName.call(this, j, e);
            if (n == "???") {
                continue
            }
            var l = [j, n, $WH.g_bonusesBtnOnChange.bind(this, (m ? "u:" : "") + j, false)];
            if (typeof m == "undefined") {
                for (var d in o[j].sub) {
                    m = o[j].sub[d].groupedUpgrade;
                    n = $WH.g_getItemBonusName.call(this, d, e, j);
                    if (n == "???") {
                        continue
                    }
                    var c = [d, n, $WH.g_bonusesBtnOnChange.bind(this, j + ":" + (m ? "u:" : "") + d, false, true)];
                    if (!l[MENU_IDX_SUB]) {
                        l[MENU_IDX_SUB] = []
                    }
            ...
            if (l[MENU_IDX_SUB] && $WH.isset("g_itembonuses")) {
                l[MENU_IDX_SUB].sort(function(q, p) {
                    var s = $WH.g_getItemBonusChanceType(q[MENU_IDX_ID]);
                    var r = $WH.g_getItemBonusChanceType(p[MENU_IDX_ID]);
                    return $WH.strcmp(s, r) || $WH.strcmp(q[MENU_IDX_NAME], p[MENU_IDX_NAME])
                });
                var h = [];
                var b = 0;
                for (var f = 0; f < l[MENU_IDX_SUB].length; ++f) {
                    var d = l[MENU_IDX_SUB][f][MENU_IDX_ID];
                    if (d && o[j].sub[d].type != b) {
                        b = o[j].sub[d].type;
                        var g = LANG.unknown;
                        switch (b) {
                            case 1:
        ...
        k.sort(function(r, p) {
            var s = r[1].innerText ? r[1].innerText : r[1];
            var q = p[1].innerText ? p[1].innerText : p[1];
            return $WH.strcmp(s, q)
        });
        a = a.concat(k)
    }
...
$WH.g_getItemBonusName = function(f, d, h) {
    var a = "";
    var e = d.level;
    if (h && $WH.isset("g_itembonuses") && h > 0 && g_itembonuses[h]) {
        for (var c = 0; c < g_itembonuses[h].length; ++c) {
            var g = g_itembonuses[h][c];
            if (g[0] == 1) {
                e += g[1];
                break
            }
    ...
    if ($WH.isset("g_itembonuses") && f > 0 && g_itembonuses[f]) {
        var b = g_itembonuses[f].slice();
        b.sort(function(k, j) {
            return k[0] - j[0]
        });
        for (var c = 0; c < b.length; ++c) {
            ...
            switch (g[0]) {
                case 1:
                    a += (a ? " / " : "") + LANG.itemlevel + " " + (e + g[1]);
                    break;
                case 2:
                    a += (a ? " / " : "") + (($WH.g_statToJson[g[1]] && LANG.traits[$WH.g_statToJson[g[1]]]) ? LANG.traits[$WH.g_statToJson[g[1]]][1] : "Unknown stat");
                    break;
                case 3:
                    a += (a ? " / " : "") + g_item_qualities[g[1]];
                    break;
                case 4:
                    if ($WH.isset("g_item_namedescriptions") && g_item_namedescriptions[g[1]]) {
                        a += (a ? " / " : "") + g_item_namedescriptions[g[1]][0]
                    }
                    break;
                case 5:
                    if ($WH.isset("g_item_namedescriptions") && g_item_namedescriptions[g[1]]) {
                        a = g_item_namedescriptions[g[1]][0]
                    }
                    break;
                case 6:
                    a += (a ? " / " : "") + g[1] + " " + g_socket_names[g[2]];
                    break;
                case 8:
                    a += (a ? " / " : "") + $WH.sprintf(LANG.tooltip_reqlevel_format.replace("%s", "$1"), d.reqlevel + g[1]);
                    break;
                case 11:
                    if (d.heirloombonuses) {
                        a += (a ? " / " : "") + $WH.sprintf(LANG.heirloomupgrade_format, (parseInt(f) == d.heirloombonuses[0]) ? 1 : 2)
                    }
                    break;
                default:
                    break
...
$WH.g_bonusesBtnOnChange = function(I, U, c) {
    ...
    for (var V = 0; V < E.length; ++V) {
        if (E[V] != K) {
            G += " + " + $WH.g_getItemBonusName.call(this, E[V], u[D], K)
        }
    }
